...
namespace oneItem {
  ...
  struct EnDis {
    using Type = bool;
    template<typename I>
    struct Part:hapi::Chain<Hidden<oneData::Default<oneData::Bool,ens>>>::template Part<I> {
      using Base=typename hapi::Chain<Hidden<oneData::Default<oneData::Bool,ens>>>::template Part<I>;
      using Base::Base;
  ...
  struct Envelope {
    template<typename... OO>
    struct Of {template<typename O> using Part=hapi::Chain<P,OO...,S>;};
  };
